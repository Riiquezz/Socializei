...
<g>
          ...
          <form onSubmit={this.handleSubmit}>
           <input type="text" name="keyword" autoComplete="off"
            required value={this.props.keyword} placeholder="Pesquisar"
    ...
    <Menu.Item className="drop" >
      ...
      <Dropdown pointing="top left" text={displayName.indexOf(' ') ? displayName.substr(0,displayName.indexOf(' ')) : displayName} >
        <Dropdown.Menu className="zidx" style={{zIndex :'100000000'}}>
          <Dropdown.Item as={Link} to={`/profile/${this.props.user.uid}`} text="Meu perfil" icon="user" />
          <Dropdown.Item  as={Link} to={`/settings`}  text="Configurações" icon="settings" />
           <Dropdown.Divider />
          <Dropdown.Item onClick={
          	()=>{ this.props.logOut()}
          } text="Sair da conta" icon="power" />
        </Dropdown.Menu>
      </Dropdown>
    </Menu.Item>
